...
{
  m_executor.execute(boost::bind(&SyncCore::handleRecoverDataExecute, this, name, content));
}

void
SyncCore::handleRecoverDataExecute(const Name &name, PcoPtr content)
{
  _LOG_DEBUG ("[" << m_log->GetLocalName () << "] <<<<< RECOVER DATA with name: " << name);

  // try to verify content with 2.0 seconds timeout
  if (! m_ccnx->verify(content, 2.0))
  {
    _LOG_ERROR ("[" << m_log->GetLocalName () << "] <<<<< RECOVER DATA with name: [" << name << "] cannot be verified");
    return;
  }
...
{
  m_executor.execute(boost::bind(&SyncCore::handleSyncDataExecute, this, name, content));
}

void
SyncCore::handleSyncDataExecute(const Name &name, PcoPtr content)
{
  ...
  if (! m_ccnx->verify(content, 2.0))
  {
    _LOG_ERROR ("[" << m_log->GetLocalName () << "] <<<<< SYNC DATA with name: [" << name << "] cannot be verified");
    return;
  }
